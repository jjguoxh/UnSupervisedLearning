...
from improved_predictor import ImprovedPatternPredictor
...
def monitor_directory_and_predict():
    ...
    监控目录并进行实时预测
    """
    logger.info("Starting directory monitoring for real-time prediction with improved model...")
    ...
    model_path = os.path.join(CURRENT_DIR, "..", "model/balanced_model/improved_predictor.pkl")
    if not predictor.load_model(model_path):
        logger.error("Failed to load improved model!")
        return
    ...
    try:
        while True:
            ...
            for data_file in data_files:
                ...
                # 进行实时预测（最后一个点）
                predicted_signal, confidence = predictor.predict(df, len(df) - 1)
                
                # 进行序列预测（最后100个点）
                sequence_predictions = []
                start_idx = max(20, len(df) - 100)  # 需要至少20个点来计算特征
                for i in range(start_idx, len(df)):
                    pred_signal, pred_conf = predictor.predict(df, i)
                ...
                single_prediction_path = os.path.join(PREDICTIONS_DIR, f"improved_realtime_prediction_{file_name}.json")
                ...
                # 保存序列预测结果
                sequence_prediction_path = os.path.join(PREDICTIONS_DIR, f"improved_realtime_sequence_prediction_{file_name}.json")
                sequence_results = {
                    'file': data_file,
                ...
                # 生成可视化结果
                output_path = os.path.join(VISUALIZATION_DIR, f"improved_realtime_prediction_{file_name}.png")
...
def visualize_realtime_predictions(df, predictions, output_path=None):
    ...
    # 设置标题
    plt.title('改进模型的实时预测结果可视化', fontsize=16)
...
def simulate_realtime_data_stream():
    ...
    try:
        for i in range(max(20, len(df) - 100), len(df)):
            ...
            # 进行预测
            predicted_signal, confidence = predictor.predict(current_df, len(current_df) - 1)
    ...
    while True:
        try:
            ...
            if choice == "1":
                ...
                # 进行序列预测（增加预测点数）
                sequence_predictions = []
                start_idx = max(20, min(200, len(df)) - 100)  # 需要至少20个点来计算特征
                for j in range(start_idx, min(start_idx + 200, len(df))):
                    pred_signal, pred_conf = predictor.predict(df, j)
                    sequence_predictions.append({
                        'index': j,
                        'predicted_signal': pred_signal,
                        'confidence': pred_conf
                ...
                if save_choice == 'y':
                    ...
                    output_path = os.path.join(PREDICTIONS_DIR, f"improved_interactive_prediction_{file_name}.json")
                    result = {
                        'file': file_path,
                        'predicted_signal': int(predicted_signal),
                        'confidence': float(confidence),
                ...
                try:
                    file_name = os.path.splitext(os.path.basename(file_path))[0]
                    visualization_path = os.path.join(VISUALIZATION_DIR, f"improved_interactive_prediction_{file_name}.png")
                    visualize_realtime_predictions(df, sequence_predictions, visualization_path)
...
def main():
    ...
    主函数
    """
    parser = argparse.ArgumentParser(description='使用改进模型的实时预测程序')
