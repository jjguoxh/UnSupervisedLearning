...
def analyze_training_data_bias():
    ...
    # 统计所有标签
    all_labels = []
    for file_path in label_files[:20]:  # 分析前20个文件
        try:
    ...
    # 检查是否存在标签分布不均的问题
    short_open_ratio = label_counts.get(3, 0) / total_labels if total_labels > 0 else 0
    long_open_ratio = label_counts.get(1, 0) / total_labels if total_labels > 0 else 0
    no_action_ratio = label_counts.get(0, 0) / total_labels if total_labels > 0 else 0
    
    print(f"\n关键标签比例:")
    print(f"  做空开仓(3): {short_open_ratio:.2%}")
    print(f"  做多开仓(1): {long_open_ratio:.2%}")
    print(f"  无操作(0): {no_action_ratio:.2%}")
    
    if short_open_ratio > 0.3:
        print("⚠️  警告: 做空开仓标签在训练数据中占比过高，可能导致模型偏向预测此类信号")
...
def analyze_model_patterns():
    ...
    try:
        ...
        total_no_action = df['no_action'].sum()
        
        total_signals = total_long_open + total_long_close + total_short_open + total_short_close + total_no_action
        
        print(f"\n模式中的信号分布:")
        print(f"  做多开仓: {total_long_open} ({total_long_open/total_signals:.2%})")
        print(f"  做多平仓: {total_long_close} ({total_long_close/total_signals:.2%})")
        print(f"  做空开仓: {total_short_open} ({total_short_open/total_signals:.2%})")
        print(f"  做空平仓: {total_short_close} ({total_short_close/total_signals:.2%})")
        print(f"  无操作: {total_no_action} ({total_no_action/total_signals:.2%})")
...
def diagnose_prediction_issues():
    ...
    # 1. 检查标签分布
    label_counts = analyze_training_data_bias()
    
    # 2. 检查预测结果
    pred_counts = analyze_prediction_bias()
