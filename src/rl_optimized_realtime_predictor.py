    ...
    try:
        while True:
            ...
            for data_file in data_files:
                ...
                single_prediction_path = os.path.join(PREDICTIONS_DIR, f"rl_optimized_realtime_prediction_{file_name}.json")
                ...
                # 保存序列预测结果
                sequence_prediction_path = os.path.join(PREDICTIONS_DIR, f"rl_optimized_realtime_sequence_prediction_{file_name}.json")
                sequence_results = {
                    'file': data_file,
                ...
                # 生成可视化结果
                output_path = os.path.join(VISUALIZATION_DIR, f"rl_optimized_realtime_prediction_{file_name}.png")
...
def visualize_realtime_predictions(df, predictions, output_path=None):
    ...
    # 设置标题
    plt.title('强化学习优化的实时预测结果可视化（合并连续开仓信号）', fontsize=16)
        ...
        try:
            ...
            if choice == "1":
                ...
                if save_choice == 'y':
                    ...
                    output_path = os.path.join(PREDICTIONS_DIR, f"rl_optimized_interactive_prediction_{file_name}.json")
                    result = {
                        'file': file_path,
                        'predicted_signal': int(predicted_signal),
                        'confidence': float(confidence),
...
def main():
    ...
    主函数
    """
    parser = argparse.ArgumentParser(description='强化学习优化的实时预测程序')
